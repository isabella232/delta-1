  ...
  test("delete expired logs") {
    withTempDir { tempDir =>
      ...
      val expectedFiles = Seq("04.json") ++ FileNames.checkpointFileWithParts(log.logPath, 4L,
        log.numberOfCheckpointParts).map(_.getName)
  ...
  test("log files being already deleted shouldn't fail log deletion job") {
    withTempDir { tempDir =>
      ...
      (1 to 25).foreach { i =>
        ...
        FileNames.checkpointFileWithParts(log.logPath, version, log.numberOfCheckpointParts)
      ...
      assert(getCheckpointFiles(logPath).length ===
